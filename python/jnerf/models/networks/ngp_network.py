...
class NGPNetworks(nn.Module):
    def __init__(self, use_fully=True, density_hidden_layer=1, density_n_neurons=64, rgb_hidden_layer=2, rgb_n_neurons=64):
        ...
        if self.use_fully and jt.flags.cuda_archs[0] >= 75 and self.using_fp16:
            ...
            density_list = [self.pos_encoder.out_dim, 16]
            rgb_list = [self.dir_encoder.out_dim+16, 3]
            for _ in range(density_hidden_layer):
                density_list.insert(1, density_n_neurons)
            for _ in range(rgb_hidden_layer):
                rgb_list.insert(1, rgb_n_neurons)
            self.density_mlp = FMLP(density_list)
            self.rgb_mlp = FMLP(rgb_list)
        else:
