...
class Runner():
    def __init__(self):
        ...
        self.save_test_result_dir = self.cfg.save_test_result_dir
        print(self.save_test_result_dir)
        print(self.cfg.task)
        self.dataset            = {}
        ...
        self.fine_tune_iters    = self.cfg.fine_tune_iters
    ...
    def train(self):
        best_psnr = 0.
        writer = SummaryWriter('logs/'+self.cfg.exp_name)
        for i in tqdm(range(self.start, self.tot_train_steps)):
            ...
            if i>0 and i%self.val_freq==0:
                ...
                writer.add_scalar(self.cfg.exp_name + '/LOSS', loss.mean().item(), i)
                writer.add_scalar(self.cfg.exp_name + '/PSNR', psnr.item(), i)
                if psnr > best_psnr:
                    best_psnr = psnr
                    self.save_ckpt(os.path.join(self.save_path, "params.pkl"))
                    print(f'Saved {os.path.join(self.save_path, "params.pkl")} successfully!')
        self.test()

    def fine_tune(self):
        assert os.path.exists(self.ckpt_path), "ckpt file does not exist: "+self.ckpt_path
        self.tot_train_steps += self.fine_tune_iters
        self.load_ckpt(self.ckpt_path)
        self.train()
    ...
    def test(self, load_ckpt=False):
        ...
        save_path = os.path.join(self.save_test_result_dir, 'result', self.exp_name)
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        mse_list=self.render_test(save_path=save_path)
    ...
    def render_img(self, dataset_mode="train", img_id=0):
        W, H = self.image_resolutions
        H = int(H)
        W = int(W)
        if img_id is None:
    ...
    def get_video(self, save_path=None, ax=1):

        if save_path is None or save_path=="":
            save_path = os.path.join(self.save_test_result_dir, 'result', self.exp_name)
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            gt_save_path = os.path.join(save_path, f"gt_{self.exp_name}.mp4")
            ge_save_path = os.path.join(save_path, f"{self.exp_name}.mp4")
            co_save_path = os.path.join(save_path, f"combine_{self.exp_name}.mp4")
        else:
            assert save_path.endswith(".mp4"), "suffix of save_path need to be .mp4"

        fps = 28
        W, H = self.image_resolutions
        gt_fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        ge_fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        co_fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        gt_videowriter = cv2.VideoWriter(gt_save_path, gt_fourcc, fps, (W, H))
        ge_videowriter = cv2.VideoWriter(ge_save_path, ge_fourcc, fps, (W, H))
        co_videowriter = cv2.VideoWriter(co_save_path, co_fourcc, fps, (W*2, H))

        print("rendering video with specified camera path")
        self.load_ckpt(self.ckpt_path)
        for img_i in tqdm(range(self.cfg.render_start_id, self.cfg.render_end_id+1, 1)):
            with jt.no_grad():
                img, img_tar = self.render_img(dataset_mode="train", img_id=img_i)
            ...
            if isinstance(img_tar, np.ndarray):
                gt_ndarr = (img_tar*255+0.5).clip(0, 255).astype('uint8')
            elif isinstance(img, jt.Var):
                gt_ndarr = (img_tar*255+0.5).clamp(0, 255).uint8().numpy()

            co_img = np.concatenate((ndarr, gt_ndarr), ax)
            ge_img = cv2.cvtColor(ndarr, cv2.COLOR_BGR2RGB)
            gt_img = cv2.cvtColor(gt_ndarr, cv2.COLOR_BGR2RGB)
            co_img = cv2.cvtColor(co_img, cv2.COLOR_BGR2RGB)

            co_videowriter.write(co_img)
            ge_videowriter.write(ge_img)
            gt_videowriter.write(gt_img)
        ge_videowriter.release()
        gt_videowriter.release()
        co_videowriter.release()
